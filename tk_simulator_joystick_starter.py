...
class GUIpart(object):
    ...
    def drawGrid(self, event=None):
        ...
        num_y = y2 / del_y
        # draw center (0,0)
        self.rCanvas.create_rectangle(canvas_width - 3, canvas_height - 3, canvas_width + 3, canvas_height + 3,
                                      fill="red")
        # horizontal grid
    ...
    def exit_prog(self, event=None):
        ...
        for robot in gRobotList:
            robot.reset()
        return
    def draw_virtual_world(self):  # keep updating the world
...
class VirtualHamsterWorld(object):
    ...
    def update_virtual_world(self):
        ...
        # how many pixels I want to update. 1mm -> 1px/1ms
        d_factor = 1.15  # travel distance conversion
        #################################

        # a_factor = 17.5 #turning speed of -15 vs angle
        distance_between_two_wheels = 35  # distance in mm, between two wheels

        vrobot = self.vworld.vrobot

        # calculate new robot's location by assuming t=0
        while not self.gQuit:
            ...
            new_vrobot_a = vrobot.a + (vrobot.sl - vrobot.sr) * del_t / distance_between_two_wheels
            ...
            # of its pose.
            # self.vworld.in_collision(new_vrobot_a, new_vrobot_x, new_vrobot_y)
            ##########################
            ...
            if (self.vworld.real_robot):  # convert prox sensor readings to number of pixels for the red beams
                ...
                if (prox_l > noise_prox):
                    ...
                    if (vrobot.dist_l and vrobot.sl == 0 and vrobot.sr == 0):
                        vrobot.dist_l = vrobot.dist_l * 2.0 + i * 10 + 10 - (prox_l - prox_conv_l[i]) * 10 / (
                                    prox_conv_l[i - 1] - prox_conv_l[i])
                        vrobot.dist_l = vrobot.dist_l / 3.0
                    else:
                        vrobot.dist_l = i * 10 + 10 - (prox_l - prox_conv_l[i]) * 10 / (
                                    prox_conv_l[i - 1] - prox_conv_l[i])
                else:
                    vrobot.dist_l = False
                ...
                if (prox_r > noise_prox):
                    ...
                    if (vrobot.dist_r and vrobot.sl == 0 and vrobot.sr == 0):
                        vrobot.dist_r = vrobot.dist_r * 2.0 + i * 10 + 10 - (prox_r - prox_conv_r[i]) * 10 / (
                                    prox_conv_r[i - 1] - prox_conv_r[i])
                        vrobot.dist_r = vrobot.dist_r / 3.0
                    else:
                        vrobot.dist_r = i * 10 + 10 - (prox_r - prox_conv_r[i]) * 10 / (
                                    prox_conv_r[i - 1] - prox_conv_r[i])
                else:
                    vrobot.dist_r = False
